                    ...
                    if (this.subscribes[urlHash].state) {
                        for (let i = 0; i < this.subscribes[urlHash].state.length; i++) {
                            ...
                            if (!this.isPatternSubscribedElsewhere(pattern, 'state', urlHash)) {
                                this.adapter.log.debug(`[${this.subscribes[urlHash].urlHook}] unsubscribe from state: ${pattern}`);
                                await this.adapter.unsubscribeForeignStatesAsync(pattern);
                            }
                            else {
                                this.adapter.log.debug(`[${this.subscribes[urlHash].urlHook}] keep subscription for state: ${pattern} (still used elsewhere)`);
                            }
                        }
                    }
                    if (this.subscribes[urlHash].object) {
                        for (let i = 0; i < this.subscribes[urlHash].object.length; i++) {
                            ...
                            if (!this.isPatternSubscribedElsewhere(pattern, 'object', urlHash)) {
                                this.adapter.log.debug(`[${this.subscribes[urlHash].urlHook}] unsubscribe from object: ${pattern}`);
                                await this.adapter.unsubscribeForeignObjectsAsync(pattern);
                            }
                            else {
                                this.adapter.log.debug(`[${this.subscribes[urlHash].urlHook}] keep subscription for object: ${pattern} (still used elsewhere)`);
                            }
                        }
                    }
            ...
            if (id) {
                ...
                do {
                    ...
                    if (pos !== -1) {
                        ...
                        if (!stillUsed) {
                            ...
                            this.adapter.log.debug(`Keep subscription for ${type} "${id}" (still used by another session)`);
                        }
                    }
                    else {
                        break;
                ...
                for (let i = 0; i < ids.length; i++) {
                    ...
                    if (!stillUsed) {
                        ...
                        this.adapter.log.debug(`Keep subscription for ${type} "${pattern}" (still used by another session)`);
                    }
                }
